...
export function ResultView() {
  ...
  // Save to S3 + DynamoDB on mount (once per generation)
  useEffect(() => {
    if (
      originalImage &&
      generatedImage &&
      !dbLogCreated.current &&
      !dbRecordId
    ) {
      dbLogCreated.current = true;
      setDbRecordId("pending_upload"); // Temporarily mark as pending to avoid parallel runs
  ...
  const handleSubmitFeedback = async () => {
    ...
    if (!dbRecordId || dbRecordId === "pending_upload") {
  ...
  const handleDownloadPdf = async () => {
    ...
    setPdfMessage("Iniciando...");

    try {
      await PdfService.generateVectorizedPdf(generatedImage, (msg) => {
        setPdfMessage(msg);
      });
      // Allow user to see "¡Listo!" briefly
      setTimeout(() => {
        setIsPdfGenerating(false);
      }, 2000);
    } catch (err) {
      console.error(err);
      alert("Hubo un error al generar el PDF/Vector. Intenta de nuevo.");
      setIsPdfGenerating(false);
    }
  };
    ...
    <div className="result-view">
      ...
      <div className="result-actions">
        ...
        <button
          ...
          title="Descargar Vector PDF"
          disabled={isPdfGenerating}
          style={{
            width: isPdfGenerating ? "auto" : undefined,
            padding: isPdfGenerating ? "0 1rem" : undefined,
            borderRadius: isPdfGenerating ? "20px" : undefined,
          }}
        >
          {isPdfGenerating ? (
